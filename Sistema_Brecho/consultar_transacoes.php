...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    try {
        $stmt = $pdo->prepare("SELECT t.*, r.modelo, rr.modelo as roupa_recebida, c.nome as cliente_nome 
                              FROM transacao t
        ...
        echo "Erro ao consultar transação: " . $e->getMessage();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    $id = $_POST['id'];
    try {
        $stmt = $pdo->prepare("DELETE FROM transacao WHERE idtransacao = ?");
        if ($stmt->execute([$id])) {
            header('location: transacoes.php?excluir=true');
        } else {
            header('location: transacoes.php?excluir=false');
        }
    } catch (\Exception $e) {
...
<form method="post" id="formExcluir">
    ...
    <div class="mb-3">
        ...
        <input disabled value="<?= ucfirst($transacao['tipo']) ?>" type="text" id="tipo" name="tipo" class="form-control">
    </div>
    <div class="mb-3">
        <label for="data" class="form-label">Data:</label>
        <input disabled value="<?= date('d/m/Y H:i', strtotime($transacao['data_transacao'])) ?>" type="text" id="data" class="form-control">
    </div>
    <div class="mb-3">
        <label for="roupa" class="form-label">Roupa:</label>
        <input disabled value="<?= $transacao['modelo'] ?>" type="text" id="roupa" class="form-control">
    </div>
    <?php if ($transacao['tipo'] == 'troca' && $transacao['roupa_recebida']): ?>
    <div class="mb-3">
        <label for="roupa_recebida" class="form-label">Roupa Recebida:</label>
        <input disabled value="<?= $transacao['roupa_recebida'] ?>" type="text" id="roupa_recebida" class="form-control">
    </div>
    <?php endif; ?>
    <div class="mb-3">
        <label for="cliente" class="form-label">Cliente:</label>
        <input disabled value="<?= $transacao['cliente_nome'] ?>" type="text" id="cliente" class="form-control">
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Tem certeza que deseja excluir a transação de "<strong><?= ucfirst($transacao['tipo']) ?></strong>"?</p>
